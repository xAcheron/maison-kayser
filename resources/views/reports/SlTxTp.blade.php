...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <h4 class="card-title">Sales, Tax, Service
                <small> - Filters</small>
            </h4>
            </div>
            <div class="card-body">
...
$(function() {
  ...
  $('#runReport').on("click",function(e){
    if(validParams())
    {
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('getReport',['id' => 3, 'format' =>'json']) }}",
            data: params,
...
{
    clearTable();
    let htmlTemplate = "<td class=\"text-left\"></td><td class=\"text-left\">:fecha</td><td class=\"text-right\">:netSales</td><td class=\"text-right\">:amountChecks</td><td class=\"text-right\">:serviceCharges</td><td class=\"text-right\">:taxCollected</td><td class=\"text-right\">:totalDiscount</td>";
    ...
    for(var i=0;i<data.length;i++)
    {
        ...
        if(data[i].fecha == "Total")
        {
            innerHTML = innerHTML.replace(':fecha', "<b>"+data[i].fecha+"</b>");
            innerHTML = innerHTML.replace(':netSales', "<b>"+formatter.format(data[i].netSales)+"</b>");
            innerHTML = innerHTML.replace(':amountChecks', "<b>"+formatter.format(data[i].amountChecks)+"</b>");
            innerHTML = innerHTML.replace(':serviceCharges', "<b>"+formatter.format(data[i].serviceCharges)+"</b>");
            innerHTML = innerHTML.replace(':taxCollected', "<b>"+formatter.format(data[i].taxCollected)+"</b>");
            innerHTML = innerHTML.replace(':totalDiscount', "<b>"+formatter.format(data[i].totalDiscount)+"</b>");
        }
        else
        {
            innerHTML = innerHTML.replace(':fecha', data[i].fecha);
            innerHTML = innerHTML.replace(':netSales', formatter.format(data[i].netSales));
            innerHTML = innerHTML.replace(':amountChecks', formatter.format(data[i].amountChecks));
            innerHTML = innerHTML.replace(':serviceCharges', formatter.format(data[i].serviceCharges));
            innerHTML = innerHTML.replace(':taxCollected', formatter.format(data[i].taxCollected));
            innerHTML = innerHTML.replace(':totalDiscount', formatter.format(data[i].totalDiscount));
        }
