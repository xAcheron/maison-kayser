...
class PmixReporter implements iReporter {
    ...
    {
        $sql = "SELECT S.*, empresas.comun, S.idEmpresa ,T.tier FROM sucursales S LEFT JOIN sucursales_tier T ON S.idTier = T.idTier INNER JOIN empresas ON empresas.idEmpresa = S.idEmpresa WHERE ".(empty($this->tier) || $this->tier == "null" || $this->tier ==0 ?"": " S.idTier = ".$this->tier." AND ")." S.idEmpresa = ?;";
    ...
    public function runReport(){
        $sql = "SELECT VT.*, F.family,MP.itemName ,COALESCE(rp.costo,0) * VT.preference AS cost, ((COALESCE(rp.costo,0) * VT.preference)/VT.netSales)*100 costoper, (VT.netSales - (COALESCE(rp.costo,0) * VT.preference)) margin , 0 perFamily, 0 perMajor, 0 perMenu, COALESCE(C.idMajor, 0)idMajor , COALESCE(C.idFamily, 0) idFamily  FROM (
            SELECT Z.idArticulo, SUM(IF(Z.idDayPart=1, Z.quantity,0)) AS breakfast, SUM(IF(Z.idDayPart=2, Z.quantity,0)) AS Lunch, SUM(IF(Z.idDayPart=3, Z.quantity,0)) AS Dinner, SUM(IF(Z.idDayPart=4, Z.quantity,0)) AS night, SUM(Z.quantity) AS preference, SUM(Z.netSales) netSales
